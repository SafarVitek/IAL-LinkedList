...
void List_Init( List *list ) {
    list->activeElement = NULL; //nastavení ukazatelů na NULL
...
void List_Dispose( List *list ) {
    while(list->firstElement != NULL) { //Cyklus který nastavý druhý prvek v pořadí na první a smaže předchozí
...
void List_InsertFirst( List *list, int data ) {
    ListElementPtr tmp = malloc(sizeof(struct ListElement)); //Alokování nového prvku
    if(tmp == NULL){ //Test zda malloc proběhl správně
        List_Error();
        return;
    }
    tmp->data = data;
    if(list->firstElement == NULL){ //Test zda se jedná o úplně první prvek
        tmp->nextElement = NULL;
        list->firstElement = tmp;
    }else{ //Pokud ne, tak se musí předchodí první element napojit na nový prvek
...
void List_GetFirst( List *list, int *dataPtr ) {
    if(list->firstElement == NULL){ //test zda seznam obsahuje alespoň jeden prvek
        List_Error();
        return;
    }
    *dataPtr = list->firstElement->data; //získání data
}
...
void List_DeleteFirst( List *list ) {
    if(list->firstElement != NULL){ //test zda je seznam prázdný
            ...
            if(tmp == list->activeElement) {
                list->activeElement = NULL; //nastavení activeElement na NULL, aby naukazoval na prázdné místo v paměti
            }
            free(tmp);
    }
}
...
void List_InsertAfter( List *list, int data ) {
    if(list->activeElement != NULL){
        ListElementPtr tmp = malloc(sizeof(struct ListElement)); //alokace prvku
        if(tmp == NULL){
            List_Error();
            return;
        }
...
void List_GetValue( List *list, int *dataPtr ) {
    ...
    *dataPtr = list->activeElement->data; //získání hodnoty
}
...
void List_SetValue( List *list, int data ) {
    if(list->activeElement != NULL){
        list->activeElement->data = data; //nastavení hodnoty, pouze pokud je seznam aktivní
    }
}
...
void List_Next( List *list ) {
    if(list->activeElement != NULL){ //posunutí activeElement na další, pouze pokud je seznam aktivní
