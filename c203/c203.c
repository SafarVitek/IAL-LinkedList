...
void Queue_Init( Queue *queue ) {
    ...
    queue->firstIndex = 0;
    queue->freeIndex = 0;
    for (int i = 0; i < QUEUE_SIZE; ++i) { //nastavení všech míst v poli na '*'
        queue->array[i] = '*';
    }
}
...
int nextIndex( int index ) {
    return (index + 1) % QUEUE_SIZE; //zvýšení indexu a následné modulo jeho zvýšení, aby nepřekročil velikost pole
}
...
int Queue_IsEmpty( const Queue *queue ) {
	return queue->firstIndex == queue->freeIndex; //pokud je firstIndex a freeIndex roven, tak je pole prázdné
}
 ...
 * @param queue Ukazatel na inicializovanou strukturu fronty
 */
int Queue_IsFull( const Queue *queue ) { //TODO nefunkční
	return nextIndex(queue->freeIndex) == queue->firstIndex;
    //pokud je další volný index už zabraný (roven firstIndex), tak je pole plné
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
    ...
    *dataPtr = queue->array[queue->firstIndex]; //získání prvku na začátku pole
}
...
void Queue_Remove( Queue *queue ) {
    ...
    queue->firstIndex = nextIndex(queue->firstIndex); //posunutí firstIndex na další místo v poli
}
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
    ...
    Queue_Front(queue, dataPtr); //získání prvku a uložení do *dataPtr
    Queue_Remove(queue); //smazání prvku v poli
}
...
void Queue_Enqueue( Queue *queue, char data ) {
    ...
    queue->array[queue->freeIndex] = data; //přiřazení hodnoty na první volné místo v poli (freeIndex)
    queue->freeIndex = nextIndex(queue->freeIndex); //posunutí freeIndex
}
